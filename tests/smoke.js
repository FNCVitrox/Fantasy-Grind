...
assert(html.includes('id="characterClassIcon"') && css.includes(".class-icon-warrior") && css.includes(".class-icon-mage") && css.includes(".class-icon-rogue") && css.includes(".class-icon-archer"), "all playable classes should have distinct portrait icons");
...
assert.strictEqual(vm.runInContext("state = defaultState(); state.characterClass = 'mage'; classResource().max", context), 6);
...
assert(vm.runInContext("state = defaultState(); state.characterClass = 'warrior'; buildName('tank') === 'Wächter' && buildName('damage') === 'Berserker' && buildName('bruiser') === 'Kriegsveteran'", context), "warrior build names should use the new class fantasy");
assert(vm.runInContext("state = defaultState(); state.language = 'en'; state.characterClass = 'mage'; entityName('class', 'mage', classCatalog.mage.name) === 'Mage' && buildName('damage') === 'Fire Mage' && buildDescription('damage').includes('fire spells') && entityName('ability', 'arcaneBolt', abilityCatalog.arcaneBolt.name) === 'Fireball'", context), "mage class, build descriptions and abilities should translate to English");
assert(vm.runInContext("state = defaultState(); state.language = 'en'; state.characterClass = 'rogue'; entityName('class', 'rogue', classCatalog.rogue.name) === 'Rogue' && buildName('damage') === 'Assassin' && buildDescription('damage').includes('crit damage') && entityName('ability', 'finisher', abilityCatalog.finisher.name) === 'Deadly Stab'", context), "rogue class, build descriptions and abilities should translate to English");
assert(vm.runInContext("state = defaultState(); state.language = 'en'; state.characterClass = 'archer'; entityName('class', 'archer', classCatalog.archer.name) === 'Archer' && buildName('damage') === 'Sharpshooter' && buildDescription('damage').includes('extremely strong hits') && entityName('ability', 'powerShot', abilityCatalog.powerShot.name) === 'Precision Shot'", context), "archer class, build descriptions and abilities should translate to English");
...
assert(vm.runInContext("(() => { state = defaultState(); state.language = 'en'; state.build = 'bruiser'; const text = knownClassAbilities().map(([id, ability]) => `${entityName('ability', id, ability.name)} ${entityText('ability', id, ability.text)}`).join(' | '); return text.includes('Battle Frenzy') && text.includes('Crush') && text.includes('Titan Strike') && !text.includes('Kampfrausch') && !text.includes('Zertrümmern') && !text.includes('Titanenhieb'); })()", context), "bruiser abilities should render English names and descriptions");
...
assert.strictEqual(vm.runInContext("state = defaultState(); state.level = 10; state.build = 'damage'; combatActionList({ awaitingPlayerAction: true, resource: 6, maxResource: 6, cooldowns: {} }).find((action) => action.abilityId === 'execute').cost", context), 6);
...
assert(vm.runInContext("(() => { state = defaultState(); state.level = 1; const combat = { awaitingPlayerAction: false, resource: 2, maxResource: 6, cooldowns: {}, playerHp: 20, combatStats: { maxHp: 100 }, enemyHp: 44, enemy: enemies.wolf, lastActionId: 'defend' }; return chooseAutoCombatAction(combat).id === 'attack'; })()", context), "auto combat should not loop defend forever at low health");
assert(vm.runInContext("(() => { state = defaultState(); const maxHp = state.maxHp; applyActionCombatLoss({ enemy: enemies.wolf, enemyName: 'Waldwolf' }); return state.hp > 0 && state.hp < state.maxHp && state.hp === Math.floor(maxHp * 0.35); })()", context), "combat defeat should return the player wounded instead of fully healed");
